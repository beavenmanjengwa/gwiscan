...
#                                                                                                  #
# The search is two hmmscan passes, so the required domains are never scanned genome-wide:          #
#   1. hmmscan the whole proteome against the PRIMARY HMM(s)  -> candidate proteins.                #
#   2. hmmscan only those candidates against the primary+required HMMs -> keep the ones that also   #
...
# hmmscan --domtblout fixed columns (0-based); mirror hmm.py.
_COL = {"target_name": 0, "target_acc": 1, "query_name": 3,
...
def read_rules(path) -> list[dict]:
    ...
    Columns (TAB-separated; '#' comment lines and blanks ignored):
      ...
      * Primary      — the defining Pfam domain hmmscan seeds the search with. One
    ...
    Both Primary and Required are Pfam accessions (PFxxxxx) — the HMMs hmmscan
    presses and searches.
    """
    rules, seen = [], set()
...
def _parse_domtbl(path):
    """hmmscan --domtblout -> list of hit dicts (protein/name/pfam/evalue/bitscore/start/end)."""
    hits = []
    with open(path) as fh:
        for line in fh:
            ...
            if len(c) < _MIN_COLS:
                continue
            hits.append({
                "protein": c[_COL["query_name"]],
                "name": c[_COL["target_name"]],
                "pfam": _bare(c[_COL["target_acc"]]),
                "acc": c[_COL["target_acc"]],
...
def _hmmscan(cfg: Config, hmm_db, query_fasta, tag):
    """Run hmmscan (query vs hmm_db) with Pfam gathering thresholds, return the domtbl path."""
    domtbl = cfg.result(f"arch_{tag}_domtbl.txt")
    external.log(f"[{datetime.now()}] hmmscan ({tag}) ...")
    external.run([
        "hmmscan", "--cut_ga", "--cpu", cfg.THREADS,
        "--domtblout", domtbl,
...
def preflight(cfg: Config) -> None:
    """Architecture-mode pre-flight: proteome, hmmscan/hmmpress, a parseable rules
    table naming only Pfam accessions, and the InterProScan requirement (it annotates
    the final candidates)."""
    external.log("[preflight] GWIscan architecture-mode pre-flight check")
    external.log("--------------------------------------")
    fail = False

    for binary in ("hmmscan", "hmmpress"):
...
def run(cfg: Config) -> None:
    """Two-stage hmmscan identification of the domain-combination members.
